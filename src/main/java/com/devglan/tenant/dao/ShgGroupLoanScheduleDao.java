...
public interface ShgGroupLoanScheduleDao extends JpaRepository<ShgGroupLoanScheduleEntity, Long>, JpaSpecificationExecutor<ShgGroupLoanScheduleEntity> {
	...
	@Modifying
	@Query(nativeQuery = true, value = "Update shg_group_loan_schedule set repaid=false, loan_paid=0, loan_os=principal_demand,last_paid_date=null,status=1 where shg_mtg_uid=?1 or (cbo_id=?2 and mtg_no=?3)")
 	void updateShgGroupLoanSchedule(BigInteger shgMtgUID, BigInteger cboId, Integer mtgNo);
	...
	@Query(nativeQuery = true, value = "Delete from shg_group_loan_schedule where shg_mtg_uid=?1") // -  and sub_installment_no>1
	 ...
	 @Query(nativeQuery = true, value = "Select * from shg_group_loan_schedule where cbo_id=?1 and loan_no=?2 and installment_no = ?3 and sub_installment_no=?4 and org_installment_no=?5 and status=?6")
