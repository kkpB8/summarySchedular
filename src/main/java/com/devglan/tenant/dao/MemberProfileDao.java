...
@Repository
public interface MemberProfileDao extends JpaRepository<MemberProfileEntity, BigInteger> {
...
@Query("FROM MemberProfileEntity c WHERE c.approveStatus = :approveStatus"
  ...
  @Query(value = "SELECT c.checkerRemark FROM MemberProfileEntity c WHERE c.memberId = :memberId")
    ...
    @Query("SELECT count(c) FROM MemberProfileEntity c WHERE c.memberId = :memberId AND c.isActive= :isActive")
    Integer getAcivatedMember(@Param("memberId") final BigInteger memberId,
                                         @Param("isActive") final Boolean isActive);
